...
#elif defined OPENCL
    cl_mem_with_offset last_input = input_layer.output_gpu;
    cl_mem_with_offset last_self = self_layer.output_gpu;
#endif
    for (i = l.steps-1; i >= 0; --i)
    {
        fill_gpu(l.outputs * l.batch, 0, l.state_gpu, 1);
...
#elif defined OPENCL
            input_layer.output_gpu.offset -= l.outputs*l.batch;
            self_layer.output_gpu.offset -= l.outputs*l.batch;
            axpy_gpu(l.outputs * l.batch, 1, input_layer.output_gpu, 1, l.state_gpu, 1);
            axpy_gpu(l.outputs * l.batch, 1, self_layer.output_gpu, 1, l.state_gpu, 1);
            input_layer.output_gpu.offset += l.outputs*l.batch;
            self_layer.output_gpu.offset += l.outputs*l.batch;
        ...
        if (i > 0) {
            s.delta_gpu = self_layer.delta_gpu;
            s.delta_gpu.offset -= l.outputs*l.batch;
        } else {
            s.delta_gpu.exist = false;
        }
        if (i == 0) s.delta_gpu.exist = false;
#endif
