...
#ifdef OPENCL
        layer l = net.layers[i];
        if(l.state_gpu.exist)
        {
            l.state_gpu.offset += l.outputs*b;
            fill_gpu(l.outputs, 0, l.state_gpu, 1);
        }
        if(l.h_gpu.exist)
        {
            l.h_gpu.offset += l.outputs*b;
            fill_gpu(l.outputs, 0, l.h_gpu, 1);
        }
#endif
    }
}
...
#elif defined OPENCL
    cl_set_device(&cl, net->gpu_index);
...
#elif defined OPENCL
    if(net.input_gpu.exist) cl_free(net.input_gpu);
    if(net.truth_gpu.exist) cl_free(net.truth_gpu);
#endif
}
...
void harmless_update_network_gpu(network net)
{
    ...
    for(i = 0; i < net.n; ++i){
        layer l = net.layers[i];
        if(l.weight_updates_gpu.exist) fill_gpu(l.nweights, 0, l.weight_updates_gpu, 1);
        if(l.bias_updates_gpu.exist) fill_gpu(l.nbiases, 0, l.bias_updates_gpu, 1);
        if(l.scale_updates_gpu.exist) fill_gpu(l.nbiases, 0, l.scale_updates_gpu, 1);
    }
}
...
void *train_thread(void *ptr)
{
    ...
    cl_set_device(&cl, args.net.gpu_index);
...
void pull_weights(layer l)
{
    if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
        cl_read_array(l.biases_gpu, l.bias_updates, l.n);
        cl_read_array(l.weights_gpu, l.weight_updates, l.n*l.size*l.size*l.c);
        if(l.scales) cl_read_array(l.scales_gpu, l.scale_updates, l.n);
    } else if(l.type == CONNECTED){
        cl_read_array(l.biases_gpu, l.bias_updates, l.outputs);
        cl_read_array(l.weights_gpu, l.weight_updates, l.outputs*l.inputs);
    }
}
...
void sync_layer(network *nets, int n, int j)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        cl_set_device(&cl, nets[i].gpu_index);
