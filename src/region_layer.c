...
#elif defined OPENCL
            int index = entry_index(l, b, n*l.w*l.h, 0);
            l.output_gpu.offset += index;
            activate_array_gpu(l.output_gpu, 2*l.w*l.h, LOGISTIC);
            l.output_gpu.offset -= index;
            if(l.coords > 4)
            {
                index = entry_index(l, b, n*l.w*l.h, 4);
                l.output_gpu.offset += index;
                activate_array_gpu(l.output_gpu, (l.coords - 4)*l.w*l.h, LOGISTIC);
                l.output_gpu.offset -= index;
            }
            index = entry_index(l, b, n*l.w*l.h, l.coords);
            l.output_gpu.offset += index;
            if(!l.background) activate_array_gpu(l.output_gpu,   l.w*l.h, LOGISTIC);
            l.output_gpu.offset -= index;
#endif
        }
    }
...
#endif // CUDA
{
    ...
    for (b = 0; b < l.batch; ++b)
    {
        for(n = 0; n < l.n; ++n)
        {
            ...
            int index = entry_index(l, b, n*l.w*l.h, 0);
            l.output_gpu.offset += index;
            l.delta_gpu.offset += index;
            gradient_array_gpu(l.output_gpu, 2*l.w*l.h, LOGISTIC, l.delta_gpu);
            l.output_gpu.offset -= index;
            l.delta_gpu.offset -= index;
            if(l.coords > 4)
            {
                ...
                l.output_gpu.offset += index;
                l.delta_gpu.offset += index;
                gradient_array_gpu(l.output_gpu, (l.coords - 4)*l.w*l.h, LOGISTIC, l.delta_gpu);
                l.output_gpu.offset -= index;
                l.delta_gpu.offset -= index;
            }
            index = entry_index(l, b, n*l.w*l.h, l.coords);
            l.output_gpu.offset += index;
            l.delta_gpu.offset += index;
            if(!l.background) gradient_array_gpu(l.output_gpu, l.w*l.h, LOGISTIC, l.delta_gpu);
            l.output_gpu.offset -= index;
            l.delta_gpu.offset -= index;
#endif
        }
