...
void forward_route_layer_gpu(route_layer l, network net)
{
    ...
    {
        int index = l.input_layers[i];
        cl_mem_with_offset input = net.layers[index].output_gpu;
        int input_size = l.input_sizes[i];
        for(j = 0; j < l.batch; ++j)
        {
            input.offset += j*input_size;
            l.output_gpu.offset += offset + j*l.outputs;
            copy_gpu(input_size, input, 1, l.output_gpu, 1);
            input.offset -= j*input_size;
            l.output_gpu.offset -= offset + j*l.outputs;
        }
        offset += input_size;
...
void backward_route_layer_gpu(route_layer l, network net)
{
    ...
    {
        int index = l.input_layers[i];
        cl_mem_with_offset delta = net.layers[index].delta_gpu;
        int input_size = l.input_sizes[i];
        for(j = 0; j < l.batch; ++j)
        {
            delta.offset += j*input_size;
            l.delta_gpu.offset += offset + j*l.outputs;
            axpy_gpu(input_size, 1, l.delta_gpu, 1, delta, 1);
            delta.offset -= j*input_size;
            l.delta_gpu.offset -= offset + j*l.outputs;
        }
        offset += input_size;
