...
__kernel void forward_maxpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride, int size, int pad, __global float *input, int input_offset, __global float *output, int output_offset, __global int *indexes, int indexes_offset)
{
    input = input + input_offset;
    output = output + output_offset;
    indexes = indexes + indexes_offset;
...
__kernel void backward_maxpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride, int size, int pad, __global float *delta, int delta_offset, __global float *prev_delta, int prev_delta_offset, __global int *indexes, int indexes_offset)
{
    delta = delta + delta_offset;
    prev_delta = prev_delta + prev_delta_offset;
    indexes = indexes + indexes_offset;
    int h = (in_h + 2*pad)/stride;
    int w = (in_w + 2*pad)/stride;
    int c = in_c;
